...
protected:
    ...
    //  нопка взаимодействи€ (E). —ам ассет IA_Interact назначим в BP_MyCharacter.
    UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Input")
    ...
    //  ак близко надо подойти к очагу, чтобы E сработала (в сантиметрах UE).
...
private:
    ...
    FTimerHandle RespawnTimerHandle;   // свой таймер дл€ возрождени€, чтобы не делить его с атакой

    void DrawDebugState() const;
    void Respawn();
    void Interact();                          // вызываетс€ кнопкой E
    void RestAtOchag(AActor* Ochag);          // отдых: HP, стамина, возрождение врагов
    AActor* FindNearestOchag() const;         // ищет ближайший очаг, персонажа не мен€ет
};